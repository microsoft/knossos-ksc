...
\begin{tikzpicture}
  ...
  \draw[->] [dashed] (dupm1) to[out=-90, in=+45] ($ (muld2) + (1, 1) $) to (muld2);
  ...
  \draw[->] (incAt) to ($ (incAt) + (-1.5, 0.0) $) node[left] {Vec $n$ $a$};
  \draw[->] [dashed] (indexdup) to[out=-90,in=45] (incAt);
  \draw[->] ($ (incAt) + (+1.5,  0.0) $) node[right] {$a$} to[out=180,  in=0] (incAt);
  \draw[->] ($ (incAt) + (+1.5, -0.5) $) node[right] {Vec $n$ $a$} to[out=180,in=-45] (incAt);
  ...
  \node[rectangle,draw,fill=black!25] (if) at ($ (fif) + (5, 0) $) {if -- then $f$ else $g$};
  ...
  \node[dot,minimum size=25] (nocheckf) at ($ (fif) + (0, -5) $) {$f$};
