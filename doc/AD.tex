...
\newcommand{\clet}[3]{{\mathcal L}(#1,#2,#3)}   % CL composition
\newcommand{\cif}[3]{{\mathcal IF}(#1,#2,#3)}   % CL conditional
\newcommand{\ctupx}{\rhd}   % CL tuple extension
...
\begin{array}{l}
  ...
  \quad = \quad \cl{f} = \KC{e}(x_1\tcolon\tau_1,\ldots,x_n\tcolon\tau_n)
  \end{array} \\ \\
\begin{array}{rcl}
\multicolumn{3}{l}{\mbox{If $\Gamma \vdash e:\tau$ then $\KC{e}\Gamma : T(\Gamma) \to \tau$}} \\
  \KC{k}{\Gamma} & = & \ck{k} \\
  \KC{x}{\Gamma} & = & \pi(\Gamma,x) \\
  \KC{f(e)}{\Gamma} & = & \cl{f} \clcomp \KC{e}{\Gamma} \\
  \KC{\lstinline|if|\;e_1\;e_2\;e_3}{\Gamma} & = & \lstinline|if|\;\begin{array}[t]{l}
      (\KC{e_1}{\Gamma}) \\ (\KC{e_2}{\Gamma}) \\ (\KC{e_3}{\Gamma}) \end{array} \\
