...
\newcommand{\gradf}[1]{\deriv #1}  % Full Jacobian
...
%    \\
    ...
    \mbox{Build}   & \lmbuild  & : & (\nat,\; \nat \to (s \linto t)) \to (s \linto \vector{n}{t}) \\
    \mbox{BuildT}   & \lmbuildt  & : & (\nat,\; \nat \to (t \linto s)) \to (\vector{n}{t} \linto s) & \mbox{Transpose of $\lmbuild$} \\
  \end{array}
  $$
\end{minipage}}
...
\begin{figure}
  ...
  \begin{array}{r@{\;}c@{\;}l}
    ...
    r \lmapplyR \lmbuild( n,\; \lambda i. m ) & = & \sumfun{\buildfun(n, \lambda i. r[i] \lmapplyR m)} ) \\
%   \lmtrans{\lmlam(\lambda i. m)} & = & \lmlamt(\lambda i. \lmtrans{m}) \\
%   \lmtrans{\lmlamt(\lambda i. m)} & = & \lmlam(\lambda i. \lmtrans{m}) \\
  \end{array}
  $$
...
\tom{Now some time has passed I think I can justify why the difference
  ...
  association.  There is no difference between the left and right
  association of a composition of two functions \(f \circ g\) yet we
  do expect there to be a difference between the forward and reverse
  mode derivatives of the composition.  Therefore there must be more
  to it than the order of composition.}
