...
\newcommand{\olmpair}{\;\overline{\lmpair}\;}         % Infix pairing
\newcommand{\olmjoin}{\;\overline{\lmjoin}\;}         % Infix pairing

\newcommand{\fwdDf}[1]{f'}  % Forward derivative, f'
\newcommand{\revDf}[1]{f'_R}  % Reverse derivative, f`
\newcommand{\lb}{\llbracket}
\newcommand{\rb}{\rrbracket}
\newcommand{\sel}[2]{\pi_{#1,#2}}
\newcommand{\fst}{\pi_{1,2}}
\newcommand{\snd}{\pi_{2,2}}
...
\newcommand{\lmbuildt}{\mathcal B'}             % Transposed Build linear map
...
{\bf Derivatives of built-in functions}
      ...
      \begin{array}{rcl}
        ...
        \gradf{\indexfunname} & :: & (\nat, \;\vector{t}) \to ((\nat, \; \vector{t}) \linto t) \\
        \gradf{\indexfunname}(i,v) & = & \lmzero \lmjoin \lmbuildt( \sizefun(v),
...
\hline
    ...
    \mbox{Pair}      & (\lmpair) & : & (s \linto t_1,\; s \linto t_2) \to (s \linto (t_1,t_2))
           & \mbox{Vertical juxtaposition} \\
    \mbox{Join}  & (\lmjoin) & : & (t_1 \linto s,\; t_2 \linto s) \to ((t_1,t_2) \linto s)
           & \mbox{Horizontal juxtaposition} \\
    \mbox{Transpose} & \lmtrans{\cdot} & : & (s \linto t) \to (t \linto s) & \mbox{Matrix transpose} \\[2mm]
%    \\
    \multicolumn{5}{l}{\mbox{\bf NB: We expect to have only $\lmlam / \lmlamt$ or $\lmbuild / \lmbuildt$, but not both}} \\
    \mbox{Lambda} & \lmlam & : & (\nat \to (s \linto t)) \to (s \linto (\nat \to t)) \\
    \mbox{TLambda} & \lmlamt & : & (\nat \to (t \linto s)) \to ((\nat \to t) \linto s) & \mbox{Transpose of $\lmlam$} \\
    \mbox{Build}   & \lmbuild  & : & (\nat,\; \nat \to (s \linto t)) \to (s \linto \vector{t}) \\
    \mbox{BuildT}   & \lmbuildt  & : & (\nat,\; \nat \to (t \linto s)) \to (\vector{t} \linto s) & \mbox{Transpose of $\lmbuild$} \\
  \end{array}
  $$
\end{minipage}}
...
{\bf Semantics of linear maps}
  ...
  \begin{array}{rcl}
   ...
   \lmlamt(f) \lmapply g & = & \Sigma_i (f \;i) \lmapply g(i) \\
    \lmbuild(n,\lambda i.\, m) \lmapply x & = & build(n, \lambda i.\, m \lmapply x) \\
    \lmbuildt(n,\lambda i.\, m) \lmapply x & = & \sumfun{\buildfun(n,\; \lambda i.\, m \lmapply x[i] )} \\
   \end{array}
  $$
\\[3mm]
...
\begin{array}{ll}
  \mbox{\bf Original function}   & f : S \to T \\
  & f(x) = e \\[2mm]
  \mbox{\bf Full Jacobian}       & \ogradf{f}  :  \forall a. (a \linto S, S) \to (a \linto T, T) \\
  & \mbox{\lstinline|$\ogradf{f}(\oarg{x})\;$ = $\;\ograd{e}$|} \\[2mm]
  \mbox{\bf Transposed Jacobian}   & \ogradft{f}  :  (S \linto a, S) \to (T \linto a, T) \\
  & \mbox{\lstinline|$\ogradft{f}(\oarg{x})\;$ =  $\;\olmtrans{(\ogradf{f}(\oarg{x}))}$|}  \\[2mm]
  \mbox{\bf Forward derivative}  & \fwdDf{f} : (S,S) \to (T,T) \\
   & \fwdDf{f}(x,dx) = \ogradf{f}(x,\lmone) \olmapply  dx \\[2mm]
  \mbox{\bf Reverse derivative}  & \revDf{f} : (S,T) \to (T,S) \\
  & \revDf{f}(x,dr) = \ogradft{f}(x,\lmone) \olmapply dr
\end{array}
$$
      {\bf Differentiation of an expression}
      \begin{center}
        If $e : T$ then $\ograd{e} : (a \linto T, T)$
      \end{center}
$$
      \begin{array}{rcll}
        \ograd{k} & = & (\lmzero, k) \\
        ...
        \ograd{(e_1,e_2)} & = & \ograd{e_1} \olmpair \ograd{e_2} \\
...
$$
 \begin{array}{rcl}
   (\olmapply) & : & (s \linto t, t) \to (s \to t) \\
   (m,v) \olmapply ds & = & m \lmapply ds \\[2mm]
   (\olmpair) & : & ((s \linto t_1,t_1),\; (s \linto t_2,t_2)) \\
                 && \to (s \linto (t_1,t_2),\; (t_1,t_2)) \\
    (m_1,t_1) \olmpair (m_2,t_2) & = & (m_1 \lmpair m_2, (t_1,t_2)) \\[2mm]
   (\olmjoin) & : & ((t_1 \linto s),t_1),\; (t_2 \linto s,t_2)) \\
                 && \to ((t_1,t_2) \linto s, \; (t_1,t_2)) \\
    (m_1,t_1) \olmjoin (m_2,t_2) & = & (m_1 \lmjoin m_2, t_1 + t_2) \\[2mm]
    \olmtrans{\cdot} & : & (s \linto t, t) \to (t \linto s, t)
...
$$
      \begin{array}{rcl}
        ...
        \ogradf{+}(m, (x,y)) & = & ((\lmone \lmjoin \lmone) \lmcomp m, \,x+y  ) \\[2mm]
        \ogradf{*}      & :: & \forall a. ((\real,\real), a \linto (\real,\real)) \to (\real, a \linto \real) \\
        \ogradf{*}(m, (x,y)) & = & ((\lmscalar{y} \lmjoin \lmscalar{x}) \lmcomp m, \, x*y)  \\[2mm]
%        \gradf{\sel{1}{2}}      & :: & (t,t) \to ((t,t) \linto t) \\
%        \gradf{\sel{1}{2}}(x) & = & \lmone \lmjoin \lmzero \\[2mm]
%        \gradf{\indexfunname} & :: & (\nat, \;\vector{t}) \to ((\nat, \; \vector{t}) \linto t) \\
%        \gradf{\indexfunname}(i,v) & = & \lmzero \lmjoin \lmbuildt( \sizefun(v),
