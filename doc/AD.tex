...
\newcommand{\vecl}[1]{\begin{codearray}{l}#1\end{codearray}}

% The KS language
\def\kskeyword#1{\mathtt{\textcolor{teal}{#1}}}
\def\ksid#1{\mathsf{#1}}

\def\ksVec#1{\ksid{Vec}~#1}

\def\kslet #1 = #2 #3{\kskeyword{let}\;#1 = #2\; \kskeyword{in} \;#3}
\def\ksletnl#1 = #2 #3{\vecl{
    \kskeyword{let}\;#1 = #2\; \kskeyword{in} \\
    #3
}}
\def\ksif#1#2#3{\kskeyword{if}\;#1\;#2\;#3}
\def\ksifnl#1#2#3{\vecl{\kskeyword{if}\;#1\\\quad #2\\\quad #3}}
\newcommand{\ksbuild}[3]{\ksid{build} \; #1 \; (\lambda #2.\, #3)}
\newcommand{\kssumbuild}[3]{\ksid{sumbuild} \; #1 \; (\lambda #2.\, #3)}
\newcommand{\ksunzip}[1]{\ksid{unzip}(#1)}
...
\newcommand{\pairsplit}{\bigtriangledown\!\!\!\!\!\!\bigtriangleup}
...
\hline
    ...
    \mbox{UnVCat}      & (\pairsplit) & : &   (s \linto (t_1,t_2)) \to (s \linto t_1,\; s \linto t_2)
           & \mbox{Vertical split} \\
...
\begin{array}{ll}
  ...
  & \gradf{f}(x) = \kslet {\gradf x} = {\lmone} {\grad{e}} \\[2mm]
...
$$
      \begin{array}{rcll}
        ...
        & = & \begin{array}[t]{l}
           ...
           \end{array} \\
        \grad{\mbox{\lstinline|let $\;(x,y)$ = $e_1\;$ in $\;e_2$|}}
        & = & \begin{array}[t]{l}
           \mbox{\lstinline|let $\;(x,y)\;$ = $\;e_1\;$ in|} \\
           \mbox{\lstinline|let $\;(\gradf{x},\gradf{y})\;$ = $\;\pairsplit(\grad{e_1})\;$ in|} \\
           \mbox{\lstinline|$\grad{e_2}$|}
           \end{array} \\
...
\newcommand{\fbogid}[1]{\ksid{#1}_{\mathit{fbog}}}
\newcommand{\rbogid}[1]{\ksid{#1}_{\mathit{rbog}}}
\newcommand{\gradfbog}[1]{\nabla_{\! f}\lb #1 \rb}  % grad[#1]
\newcommand{\gradrbog}[1]{\nabla_{\! r}\lb #1 \rb}  % grad[#1]
\newcommand{\gradrbogEB}[3]{\nabla_{\! r}\lb #1 \rb\;#2\;#3}  % grad[#1]
\newcommand{\gradrbogcps}[4]{\nabla_{\! r}\lb #1 \rb\;#2\;#3\;#4}
...
\begin{align*}
 ...
 &\rbog{f}(\gradf{t},b) = \mbox{\lstinline|let $\gradrbogEB{e}{\gradf{t}}{b}\;$ in $\;\gradf{s}$|}
\end{align*}
If $\Gamma \vdash e : T$ and $\gradf{t}{:}\tangent{T}$ and $b{:}\bog{e}$,
      then $\gradrbogEB{e}{\gradf{t}}{b}$ is a set of bindings that,
...
\eq<   \gradfbog{k}  === (k,())>
&
\eq<   \gradrbogEB{k}{\gradf{t}}{b}   === \{ \} >
\\
% Variable use, x
\eq<   \gradfbog{x}   ===  (x,()) >
&
\eq<   \gradrbogEB{x}{\gradf{t}}{b}    ===   \{ \gradf{x} = \gradf{t} \}   >
\\
% f(e)	
...
>
&
\eq<   \gradrbogEB{f(e)}{\gradf{t}}{b} ===  \begin{codearray}{lll}
                                        \{ & (b_e,b_f) = b \\
            ...
            ; & \gradf{\overline{v}}  =
                \begin{codearray}{l}
                   \mbox{\lstinline{sumbuild n}} \\
                      ...
                      let $\;\gradrbogEB{e_2}{(\gradf{t}[i])}{(ba[i])}\;$ in $\;\gradf{\overline{v}})$|}
                \end{codearray} & \biggr\} \\
            \end{codearray}
\end{multline*}
...
\begin{figure*}
  \small
  \def\mcol#1{\multicolumn{3}{l}{#1}}
  \def\mkhead#1{\multicolumn{3}{l}{\begin{minipage}[t]{0.45\linewidth}#1\end{minipage}}}
  \def\cps<#1===#2>{$#1$   &   $=$   &    $#2$}

\def\doexprbase#1#2#3{
  $\vecl{#1}$ & \multicolumn{2}{l}{$\vecl{#2}$} & \multicolumn{3}{l}{$\vecl{#3}$}
}

\def\doexpr#1#2#3#4#5#6{
  % Expr & Fwd & RevArgs & Rev
  \doexprbase{#1}
    {\gradfbog{e} = #2}
    {\gradrbog{e} \; #3 \; #4 \; #5 = #6}
}
\def\comment#1{
  \multicolumn{6}{l}{\parbox{\linewidth}{#1}}\\
}
...
\begin{tabular}[t]{lclrcl}

\doexprbase{
  \text{\bf Original function}\\
      f : S \to T \\
   f(s) = e
} {
  \text{\bf Forward BOG}\\
  \fbog{f} : S \to (T,\bog{e})\\
  \fbog{f}(s) = \gradfbog{e} \\
  \text{If $e : T$ then $\gradfbog{e} : (T,\bog{e})$} 
} {
  \text{\bf Reverse BOG}\\
  \rbog{f} : (\tangent{T},\bog{e}) \to \tangent{S} \\
  \rbog{f}(\gradf{t},b) = \gradrbogcps{e} \dt b {\gradf s}\\[2pt]
  \parbox{0.45\linewidth}{
    If $e : E$
        and $\de{:}\tangent{E}$ 
        and $b{:}\bog{e}$ 
        and $\body : \tangent Q$
        then $\gradrbogcps{e} \de b \body : \tangent Q$ evaluates $\body$ in
         a context where all free variables of $\body$ are bound.
  }
}
\\
Expression $e$ & \multicolumn{2}{l}{ } & \multicolumn{3}{l}{ }\\ 
\hline
% Constant, k
\doexpr k {
  (k,())
...
\\[-10pt]
\comment{
Note: Use of $\underline{x}$ becomes binding of $\underline{\gradf x}$, which will be used in $\body$.  
If $x$ is unused, it won't be bound in the reverse pass. 
If it's used twice, there will be shadowing definitions, hence incorrect gradients, so we use $\ksid{dup}$.
}
\doexpr {\underline{x}} {
  (x,())
}
\de b \body {
   \ksletnl {\underline{\gradf x}} = {\de} {
     \body
    }
  }
\\
\hline
% let x = e1 in e2
\comment{Note: Binding of $\underline{x}$ becomes use of $\underline{\gradf x}$ in the body of $e_2$'s reverse-pass.}
\doexpr{\kslet \underline{x} = {e_1} {e_2}} {
 ...
 {\de_2} {(b_1,b_2)} \body {
                          ...
                          \quad  \gradrbogcps{e_1} {\underline{\gradf x}} {b_1} \body\\
                          )
}}
\\ 
% let (x, y) = e1 in e2
\doexpr{\kslet {(\underline x,\underline y)} = {e_1} {e_2}} {
...
{\de_2} {(b_1,b_2)} \body {
  \vecl{
   \gradrbogcps{e_2}{\de_2}{b_2} (\\
 \quad    \gradrbogcps{e_1}{(\underline{\gradf x},\underline{\gradf y})}{b_1} 
      \body\\
   )}
   } 
\\
...
% f(e)	
\doexpr {f(a)} {
    \ksletnl (v_a,b_a) = {\gradfbog{a}} {
      \ksletnl (r,b_f) = {\fbog{f}(v_a)} {
        (r, (b_a,b_f))
      }
    }
  }
  \de {(b_a, b_f)} \body {
    \ksletnl{\gradf f} = {\rbog f(\de, b_a)} {
      \gradrbogcps{a} {\gradf f} {b_f} \body
    }
  }
\\
\hline
...
\de {(b_p, b_{tf})} \body {
  \ksifnl {b_p} {
    \gradrbogcps{e_t} \de {\pi_1(b_{tf})} \body
  } {
    \gradrbogcps{e_f} \de {\pi_2(b_{tf})} \body
  }
% -- But if typeof(BOG$t) == typeof(BOG$f), optimize to
% if bogp
%   R[et] de bogif body
%   R[ef] de bogif body
}
...
% build e1 (\lambda i . e2) 
\comment{Note: Build with ``inlined'' lambda has a special rule, 
build with top-level function doesn't need one (see below)}
\doexpr{\ksbuild {e_1} i {e_2}} {
  \ksletnl n = {e_1} {
    \ksletnl \boldsymbol{rb} = {\ksbuild n i {\gradfbog{e_2}}} {
      \ksletnl (\boldsymbol{r},\boldsymbol{b}) = {\ksunzip{\boldsymbol{rb}}} {
        (\boldsymbol{r}, (n,\boldsymbol{b}))
      }
    }
  }
}
\de {(n, bs)} \body {\\
  \quad \kssumbuild n i {\gradrbogcps {e_2} {\de[i]} {bs[i]} \body}
}\\
\hline
% % case e1 of { Inl x el; Inr y er }
% \mkhead{
...
% \mkhead{
% \begin{multline*}
%           \gradrbogcps{\mbox{\lstinline|case $\;e_1\;$ of
%               \{ Inl x $\rightarrow e_2$; Inr y $\rightarrow e_3$ \}|}} \de b \body =\\
%           \begin{codearray}{l}
%            \{\, (b_1,b_2) = b \\
%             ;\, (\gradf{xy}, \overline{v}) = \begin{codearray}{l}
%          \mbox{\lstinline|case $\;b_2\;$ of|} \\
%          \quad \mbox{\lstinline|Inl $\;b_l\rightarrow \;$ let $\;\gradrbogcps{e_l}{\gradf{t}}{b_l}\;$ in $\;($Inl$\;\gradf{x},\overline{v})$|} \\
%          \quad \mbox{\lstinline|Inr $\;b_r\rightarrow \;$ let $\;\gradrbogcps{e_r}{\gradf{t}}{b_r}\;$ in $\;($Inr$\;\gradf{y},\overline{v})$|}\, \} \\
%            \end{codearray} \\
%          \append \; \gradrbogcps{e_1}{\gradf{xy}}{b_1}
%          \end{codearray}
%     \end{multline*}
% }\\
\end{tabular}
\caption{BOG-style AD for ksc core language, continuation-passing style, for the single-use dialect of \ksc{},
      in which every binder is used exactly once.
} \label{fig:ksc-bog-ad-cps}

\begin{tabular}{llllll}
  \hline
  Definition & \multicolumn{2}{l}{Forward} & \multicolumn{3}{l}{Reverse}\\
  \hline
  % dup
  \doexprbase{\vecl{
    \ksid{dup} : T \to (T, T)\\
    ...
    \rbogid{dup}((\dt_1, \dt_2), ()) = \dt_1 + \dt_2
  }\\
  \hline
  % build
  \doexprbase{\vecl{
    \ksid{build} : (E, \mathbb N, (E, \mathbb N) \to T) \to \ksVec T\\
    \ksid{build}(env, n, f) = \dots\\
    f : (E, \mathbb N) \to T \\
    \fbog f : (E, \mathbb N) \to (T, \bog f)\\
    \rbog f : (\tangent{T}, \bog f) \to (\tangent{E}, ())
  }} {
    \vecl{
      \fbogid{build}(env, n, f) =\\
      \quad \ksletnl bs = {\ksid{build}(env, n, \fbog f)} {
        \ksletnl (r,ba) = {\ksid{unzip}(bs)} {
          (r, (n, ba))
        }
      }
    }
  } {
    \rbogid{build}(\mathbf\dt, (n, ba)) = \\
    \quad  \kssumbuild n i {\rbog{f}(\dt[i], ba[i])}
    }\\
  \hline
\end{tabular}
\caption{BOG-style AD for some ksc primitive functions} \label{fig:ksc-bog-ad-cps}
\end{figure*}
