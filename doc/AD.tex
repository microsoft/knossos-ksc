...
\newcommand{\clcomp}{\,\circ\,}   % CL composition
...
\renewcommand{\vector}[2]{\mathit{Vec}\;#1\;#2}   % the type (Vec n t)
...
%  \deltafun & :: & Field \; t \darrow (\nat,\; \nat) \to t & \mbox{Delta-function} \\
  \buildfun & :: & (n::\nat,\; \nat \to t) \to \vector{n}{t} & \mbox{Vector build} \\
  \indexfunname & :: & (\nat, \; \vector{n}{t}) \to t & \mbox {Indexing (NB arg order)} \\
  \sumfunname & :: & \vector{n}{t} \to t & \mbox{Sum a vector} \\
  \sizefun & :: & \vector{n}{t} \to \nat & \mbox{Size of a vector} \\
\end{array}
$$
\\[3mm]
...
\hline
    ...
    \mbox{VCatV}      & \lmvcatv{\cdot} & : & \vector{n}{(s \linto t)} \to (s \linto \vector{n}{t})
           & \mbox{...vector version} \\
    \mbox{HCat}  & (\lmhcat) & : & (t_1 \linto s,\; t_2 \linto s) \to ((t_1,t_2) \linto s)
           & \mbox{Horizontal juxtaposition} \\
    \mbox{HCatV}  & \lmhcatv{\cdot} & : & \vector{n}{(t \linto s)} \to (\vector{n}{t} \linto s)
           & \mbox{...vector version} \\
...
$$
 \begin{array}{rcl}
   ...
   (\olmhcat) & : & ((t_1, t_1 \linto s),\; (t_2, t_2 \linto s))
    ...
    \swap & : & \vector{n}{(a,b)} \to (\vector{n}{a}, \vector{n}{b}) \\[2mm]
    \olmtrans{\cdot} & : & (r, s \linto t) \to (r, t \linto s)
%   B  & : & (N, N \to (s \linto t, t)) \to (s \linto \vector{n}{t}, \vector{n}{t})
 \end{array}
$$
 \\
   {\bf Derivatives of built-in functions}
...
%               \lambda j. \mbox{\lstinline|if|}\, i=j \,\mbox{\lstinline|then|}\, \lmone  \, \mbox{\lstinline|else|}\, \lmzero ) \\[2mm]
%         \gradf{\sumfunname} & :: & Field \; t \darrow \vector{n}{t} \to (\vector{n}{t} \linto t) \\
...
$$
      \begin{array}{rcll}
        ...
        \multicolumn{4}{l}{\mbox{\bf Terms}} \\
        \mathit{pgm} & ::= & \mathit{def}_1 \ldots \mathit{def}_n \\
        \mathit{def} & ::= & f : S \to T = e \\
        c & ::= & \ck{k} & \mbox{Constant} \\
          & |   & \cfun{f} & \mbox{Function constant} \\
          & |   & \sel{i}{n} & \mbox{Selector} \\
      ...
      \end{array}
$$
\mbox{{\bf Semantics:}\; \fbox{$c \capply e = e$}}
$$
\begin{array}{r@{\hspace{1mm}}c@{\hspace{1mm}}l}
   \ck{k} \capply t & = & k \\
   \cfun{f} \capply t & = & f(t) \\
   \sel{i}{n}\capply (x_1, \ldots, x_n) & = & x_i \\
   (c_1 \clcomp c_2) \capply t & = & c_1 \capply (c_2 \capply t) \\
   (c_1,\ldots, c_n) \capply t & = & (c_1 \capply t,\ldots, c_n \capply t) \\
   \cif{c_1}{c_2}{c_3}\capply t & = & \lstinline|if|\; (c_1 \capply t) \; (c_2 \capply t)\; (c_3 \capply t) \\
   \clet{x}{c_r}{c_b} \capply t & = & \lstinline|let|\;\begin{array}[t]{l}
   x\, = \, c_r\capply t \\
   \lstinline{in}\; c_b \capply (t \ctupx x)
   \end{array}
\end{array}
$$
...
\end{array} \\
\begin{array}{rcl}
\pi((x_1\tcolon\tau_1,\ldots, x_n\tcolon\tau_n),\,x_i) & = & \sel{i}{n} \\
...
\begin{array}{l}
  ...
  \end{array} \\ \\
\begin{array}{r@{\hspace{1mm}}c@{\hspace{1mm}}l}
\multicolumn{3}{l}{\mbox{If $\Gamma \vdash e:\tau$ then $\KC{e}\Gamma : T(\Gamma) \darrow \tau$}} \\
  \KC{k}{\Gamma} & = & \ck{k} \\
  \KC{x}{\Gamma} & = & \pi(\Gamma,x) \\
  \KC{f(e)}{\Gamma} & = & \cfun{f} \clcomp \KC{e}{\Gamma} \\
  \KC{\lstinline|if|\;e_1\;e_2\;e_3}{\Gamma} \\
     \multicolumn{3}{l}{\hspace{2cm} = \lstinline|if|\,
      (\KC{e_1}{\Gamma})\, (\KC{e_2}{\Gamma})\, (\KC{e_3}{\Gamma})} \\
  ...
  \KC{\lambda x.e}{\Gamma} & = & blah
\end{array}
\\[1mm]
\mbox{\bf Pruning} \\
\begin{array}{rcl}
  \KC{e}\Gamma & = & \KC{e}{\Gamma'} \clcomp \KC{(v_1,\ldots,v_n)}{\Gamma} \\
...
\begin{figure}
  ...
  \begin{gather*}
    ...
    \infer{\Gamma \vdash \ck{k} : S \darrow \real}
          {}
   \\[1mm]
   \infer{\Gamma \vdash \sel{i}{n} : (T_1,\ldots,T_i,\ldots, T_n) \darrow T_i}{}
   \\[1mm]
   \infer{\Gamma \vdash c_1 \clcomp c_2 : S \darrow T}
   {\Gamma \vdash c_1 : R \darrow T \quad \Gamma \vdash c_2 : S \darrow R}
   \\[1mm]
