...
\def\doexpr#1#2#3#4#5#6{
  ...
  $#1$ & \multicolumn{2}{l}{$\gradfbog{e} = #2$}
       & \multicolumn{3}{l}{$\gradrbog{e} \; #3 \; #4 \; #5 = #6$}
}
\def\comment#1{
  \multicolumn{6}{l}{#1}\\
}
...
\hline
% Variable use, x
\comment{Note: Use of $\underline{x}$ becomes binding of $\underline{\gradf x}$, which will be used in $\body$}
\doexpr {\underline{x}} {
  (x,())
}
\de b \body {
   \ksletnl {\underline{\gradf x}} = {\dt} {
     \body
    }
  }
\\
\hline
% let x = e1 in e2
\comment{Note: Binding of $\underline{x}$ becomes use of $\underline{\gradf x}$, in the caller's body}
...
\hline
% if bogp
%   R[et] de (get$t bogif) body
%   R[ef] de (get$f bogif) body
...
\de {(n, bs)} \body {% continued on next line
\vecl{{}\\
...
\end{tabular}
\end{minipage}}
\caption{BOG-style AD for ksc, continuation-passing style} \label{fig:ksc-bog-ad-cps}
\end{figure*}
