...
# Cache that here.
_class_hashes: Mapping[Type[Expr], int] = {
...
def _hash_children(node_type_hash: int, e: Expr, reverse_debruijn_vars: Mapping[str, int]) -> int:
...
def _alpha_hash_const(c: Const, reverse_debruijn_vars: Mapping[str, int]) -> int:
    # System hash() is repeatable for int/float but not str.
    return _hash_str(c.value) if isinstance(c.value, str) else hash(c.value)

@_alpha_hash_helper.register
def _alpha_hash_call(c: Call, reverse_debruijn_vars: Mapping[str, int]) -> int:
