...
def find_template_subst_lam(
    ...
    if template.arg.type_ not in [exp.arg.type_, Type.Any]:
        return None
    body_subst = find_template_subst(
        template.body,
        exp.body,
        template_vars.set(template.arg.name, exp.arg.type_),
    )
...
class SubstTemplate(ExprTransformer):
    ...
    def visit(self, e: Expr, var_names_to_exprs: VariableSubstitution) -> Expr:
        ...
        if res.type_ == Type.Any:
            # Remove type, so type propagation will recalculate just one level without recursion
            res.type_ = None
            # No symtab should be required: "Any" should only be used in rules for builtins which are universally polymorphic,
            # not for ad-hoc-overloaded functions from prelude etc.
