...
class TraceableFunction:
    ...
    def __call__(self, *args):
        ...
        for i, arg in enumerate(args):
            ...
            print(
                f"Processing the {i+1}th argument to {self.name} with {converted_args[-1].shape_type}"
            )
        ...
        func_node = node.Node(
            jitted.name, st.shape, st.type, children=converted_args, jitted=jitted
        )
        value_node = node.Node(
            "_identity", st.shape, st.type, children=[func_node], data="__not_ready__"
        )
