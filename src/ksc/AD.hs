...
gradCall TupleAD s f args
  = mkLets [p | (_, Just p) <- grad_arg_binds] $
    mkLet res_tv
          (Call gf [ pFst ga | (ga,_) <- grad_arg_binds ]) $
    Tuple [ pFst (Var res_tv)
          , lmCompose (pSnd (Var res_tv))
                      (lmVCat [ pSnd ga | (ga, _) <- grad_arg_binds ])
          ]
  where
...
gradLet TupleAD s v e1 e2
  = mkLet (gradTVar TupleAD s v) (gradE TupleAD s e1) $
    gradE TupleAD s e2
