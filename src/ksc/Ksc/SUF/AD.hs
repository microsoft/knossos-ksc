...
sufFwdRevPass gst subst = \case
  ...
  --
  -- REV{if cond then t else f} dt b = { let (b_cond, cond, b_t, b_f) = b
...
--                          in (r, (n, b_n, b_a))
--
-- REV{build e1 (\i. e2)} dt b = { let (n, b_n, b_a) = b
...
sufFwdRevPassDef gst Def{ def_fun    = Fun f
                        , def_pat    = s
                        , def_rhs    = UserRhs rhs
                        , def_res_ty = t_ty
                        }
  = let
      fwd = Def { def_fun    = SUFFwdPass f
                , def_pat    = s
                , def_rhs    = UserRhs rhs'
...
sufRevDef gst Def{ def_fun    = fun@(Fun f)
                 , def_pat    = s
...
mkSufFuns env fun arg_ty = (fwdTFun, revTFun, bog_ty)
  where fwdTFun = TFun fwd_res_ty fwdFun
        revTFun = TFun rev_res_ty (SUFRevPass funid)
        fwdFun = SUFFwdPass funid
