...
#define KS_DEF
#define KS_FUNCTION
#define KS_INLINE_FUNCTION inline
#define KS_INTERFACE
...
#define KS_LEAVE { KS_LOG("dtor " << KS_FIND, --log_indent);  objects.erase(this); }
	...
	struct Tuple<T0, T1, T2, T3, T4, Ts...> {
		KS_INTERFACE Tuple() { }
		KS_INTERFACE Tuple(T0 t0, T1 t1, T2 t2, T3 t3, T4 t4, Ts ...ts) : t0(t0), t1(t1), t2(t2), t3(t3), others{t4, ts...} { }
	...
	template<size_t I> struct get_helper_t
	{
		...
		static KS_INTERFACE auto const& get(Tuple const& t) {
			return get_helper_t<I - 4>::get(t.others);
		}
	};
	template<> struct get_helper_t<0> { template<typename Tuple> static KS_INTERFACE auto const& get(Tuple const& t) { return t.t0; } };
	template<> struct get_helper_t<1> { template<typename Tuple> static KS_INTERFACE auto const& get(Tuple const& t) { return t.t1; } };
	...
	KS_INTERFACE bool Tuple_equal_impl(TupleT const& lhs, TupleT const& rhs, std::index_sequence<Indices...>) {
		return ((ks::get<Indices>(lhs) == ks::get<Indices>(rhs)) && ...);
	}
	template<typename ...Ts>
	KS_INTERFACE bool operator == (Tuple<Ts...> const& lhs, Tuple<Ts...> const& rhs) {
...
#define DECLARE_TYPE_TO_STRING_Pack(Tmpl)                         \
	...
	KS_FUNCTION auto transform_Tuple_impl(TupleT const& t, F f, std::index_sequence<Indices...>)
	{
		return ks::make_Tuple(f(ks::get<Indices>(t))...);
	}

	template<typename ...Ts, typename F>
	KS_FUNCTION auto transform_Tuple(Tuple<Ts...> const& t, F f)
	{
		return transform_Tuple_impl(t, f, std::index_sequence_for<Ts...>{});
	}

	template < typename T, typename... Ts >
	KS_INTERFACE auto head(Tuple<T, Ts...> const& t)
	{
		return  ks::get<0>(t);
	}

	template < std::size_t... Ns, typename... Ts >
	KS_INTERFACE auto tail_impl(std::index_sequence<Ns...>, Tuple<Ts...> t)
	{
		return  ks::make_Tuple(ks::get<Ns + 1u>(t)...);
	}

	template < typename... Ts >
	KS_INTERFACE auto tail(Tuple<Ts...> t)
	{
	...
	*/
	...
	struct tensor_dimension_base<std::index_sequence<Indices...>>
	{
		...
		static KS_INTERFACE int num_elements(index_type const& size) {
		...
		static KS_INTERFACE bool index_is_in_range(index_type const& i, index_type const& tensor_size) {
			return (
	...
	struct tensor_dimension : public tensor_dimension_base<std::make_index_sequence<Dim>>
	{
		...
		static KS_INTERFACE index_type tail(const HigherIndexType & i) { return ks::tail(i); }

		template<typename IndexType>
		static KS_INTERFACE int flatten_index_recursive(IndexType const& index, IndexType const& tensor_size) {
		...
		static KS_INTERFACE int flatten_index(index_type index, index_type tensor_size) {
	...
	struct tensor_dimension<1>
	{
		...
		static KS_INTERFACE int tail(ks::Tuple<int, int> higherIndexType) { return ks::get<1>(higherIndexType); }

		static KS_INTERFACE int num_elements(index_type size) { return size; }

#ifdef KS_BOUNDS_CHECK
		static std::string index_to_string(index_type i) { return std::to_string(i); }
#endif

		static KS_INTERFACE bool index_is_in_range(index_type index, index_type tensor_size) {
	...
	public:
		...
		KS_INTERFACE tensor() : size_{}, data_{ nullptr } {}
		KS_INTERFACE tensor(index_type size, T * data) : size_(size), data_(data) {}


		KS_INTERFACE static size_t bytes_required(index_type size) {
			return sizeof(T) * static_cast<size_t>(dimension::num_elements(size));
		}

		KS_INTERFACE index_type size() const { return size_; }
		KS_INTERFACE int outer_dimension() const { return get_dimension<0>(size_); }
		KS_INTERFACE int num_elements() const { return dimension::num_elements(size_); }

		KS_INTERFACE T* data() { return data_; }
		KS_INTERFACE const T* data() const { return data_; }

		KS_INTERFACE std::conditional_t<Dim == 1u, T&, tensor<Dim-1, T>> operator[](int i) {
			if constexpr (Dim == 1u) {
				return data_[i];
			} else {
				return subtensor(i);
		...
		KS_INTERFACE std::conditional_t<Dim == 1u, const T&, tensor<Dim-1, T>> operator[](int i) const {
			if constexpr (Dim == 1u) {
				return data_[i];
			} else {
				return subtensor(i);
		...
		KS_INTERFACE T const& index(index_type i) const {
			return const_cast<tensor*>(this)->index_aux(i);
		}

		KS_INTERFACE T& index(index_type i) {
			return index_aux(i);
		}
		
		KS_INTERFACE void set_if_index_is_in_range(index_type i, T const& val) {
			if (dimension::index_is_in_range(i, size_)) {
...
#ifdef KS_ALLOCATOR
		static KS_INTERFACE tensor<Dim, T> create(allocator_base * alloc, index_type size)
		{
			void *storage = alloc->allocate(bytes_required(size));
			return tensor<Dim, T>(size, (T*)storage);
		}
#else
		static KS_INTERFACE tensor<Dim, T> create(allocator_base *, index_type)
		{
		...
		KS_INTERFACE bool operator == (tensor const& other) const {
			if (size() != other.size()) {
				return false;
			}
		...
		KS_INTERFACE bool operator != (tensor const& other) const { return !(*this == other); }
	};
	...
	KS_INLINE_FUNCTION Tuple<> shape(allocator_base *, Bool const&) { return {}; }
	KS_INLINE_FUNCTION Tuple<> shape(allocator_base *, Integer const&) { return {}; }
	KS_INLINE_FUNCTION Tuple<> shape(allocator_base *, Float const&) { return {}; }
	KS_INLINE_FUNCTION Tuple<> shape(allocator_base *, String const&) { return {}; }

	template<size_t Dim, class T>
	KS_FUNCTION auto shape(allocator_base * alloc, tensor<Dim, T> const& t) {
	...
	KS_FUNCTION auto shape_impl(allocator_base * alloc, TupleType const& t, std::index_sequence<Indices...>) {
		return ks::make_Tuple(shape(alloc, ks::get<Indices>(t))...);
	}

	template<class... Types>
	KS_FUNCTION auto shape(allocator_base * alloc, Tuple<Types...> const& t) {
		return shape_impl(alloc, t, std::index_sequence_for<Types...>{});
	}

	KS_INLINE_FUNCTION auto shape(allocator_base *) { return Tuple<>{}; }

	template<class T1, class T2, class... Ts>
	KS_FUNCTION auto shape(allocator_base * alloc, T1 const& t1, T2 const& t2, Ts const& ...ts) {
	...
	template <class T>
	KS_FUNCTION T inflated_deep_copy(allocator_base *, T z)
	{
		return z;
	}

	template <class T, class... Ts>
	KS_FUNCTION Tuple<T, Ts...> inflated_deep_copy(allocator_base * alloc, Tuple<T, Ts...> val)
	{
		return transform_Tuple(val, [alloc](auto const& elem) { return inflated_deep_copy(alloc, elem); });
	}

	template <size_t Dim, class T>
	KS_FUNCTION tensor<Dim, T> inflated_deep_copy(allocator_base * alloc, tensor<Dim, T> t)
	{
	...
	template<class T>
	KS_FUNCTION size_t inflated_bytes(T const&) { return 0; }

	template<class TupleT, size_t... Indices>
	KS_FUNCTION size_t inflated_bytes_Tupleimpl(TupleT const& t, std::index_sequence<Indices...>) {
		return ((size_t)0 + ... + inflated_bytes(ks::get<Indices>(t)));
	}
	template<class... Types>
	KS_FUNCTION size_t inflated_bytes(Tuple<Types...> const& t) {
		return inflated_bytes_Tupleimpl(t, std::index_sequence_for<Types...>{});
	}

	template<size_t Dim, class T>
	KS_FUNCTION size_t inflated_bytes(tensor<Dim, T> const& t) {
	...
	// range [start, end)
	template<class T>
	KS_FUNCTION bool memory_overlaps(const void* /*start*/, const void* /*end*/, T const& /*val*/) {
		return false;
	}

	template<class TupleT, size_t... Indices>
	KS_FUNCTION bool memory_overlaps_Tupleimpl(const void* start, const void* end, TupleT const& t, std::index_sequence<Indices...>) {
		return (memory_overlaps(start, end, ks::get<Indices>(t)) || ...);
	}
	template<class... Types>
	KS_FUNCTION bool memory_overlaps(const void* start, const void* end, Tuple<Types...> const& t) {
		return memory_overlaps_Tupleimpl(start, end, t, std::index_sequence_for<Types...>{});
	}

	template<size_t Dim, class T>
	KS_FUNCTION bool memory_overlaps(const void* start, const void* end, tensor<Dim, T> const& t) {
	...
	template<class T>
	KS_FUNCTION void prepare_copydown_inplace(prepare_copydown_state *, T *) {
		/* There's nothing to do unless T has a tensor subobject. */
	}

	template<class TupleT, size_t... Indices>
	KS_FUNCTION void prepare_copydown_inplace_Tupleimpl(prepare_copydown_state * dest, TupleT * t, std::index_sequence<Indices...>) {
		((prepare_copydown_inplace(dest, &ks::get<Indices>(*t))), ...);
	}

	template<class... Types>
	KS_FUNCTION void prepare_copydown_inplace(prepare_copydown_state * dest, Tuple<Types...> * t) {
		prepare_copydown_inplace_Tupleimpl(dest, t, std::index_sequence_for<Types...>{});
	}

	template<size_t Dim, class T>
	KS_FUNCTION void prepare_copydown_inplace(prepare_copydown_state * dest, tensor<Dim, T> * t) {
	...
	template<class T>
	KS_FUNCTION T prepare_copydown(allocator * alloc, alloc_mark_t mark, T val) {
	...
	template<class T>
	KS_FUNCTION void copydown_by_memmove_inplace(allocator *, T *) { }

	template<class TupleType, size_t... Indices>
	KS_FUNCTION void copydown_by_memmove_inplace_Tuple(allocator * alloc, TupleType * t, std::index_sequence<Indices...>) {
		((copydown_by_memmove_inplace(alloc, &ks::get<Indices>(*t))), ...);
	}
	template<class... Types>
	KS_FUNCTION void copydown_by_memmove_inplace(allocator * alloc, Tuple<Types...> * t) {
		copydown_by_memmove_inplace_Tuple(alloc, t, std::index_sequence_for<Types...>{});
	}

	template<size_t Dim, class T>
	KS_FUNCTION void copydown_by_memmove_inplace(allocator * alloc, tensor<Dim, T> * t) {
		int num_elements = t->num_elements();
		T* oldData = t->data();
	...
	/* Perform a copydown by iterating over the subobjects of val;
	   ...
	   */
	template<class T>
	KS_FUNCTION T copydown_by_memmove(allocator * alloc, alloc_mark_t mark, T val)
	{
		alloc->reset(mark);
	...
	template<class T>
	KS_FUNCTION T copydown(allocator * alloc, alloc_mark_t mark, T const& val)
	{
	...
	template <class T>
	KS_FUNCTION T zero(allocator * alloc, T const& val)
	{
	...
	template <>
	KS_INLINE_FUNCTION Float zero(allocator *, Float const& val)
	{
		return 0.0;
	}

	template <>
	KS_INLINE_FUNCTION Integer zero(allocator *, Integer const& val)
	{
		return 0;
	}

	template <class... Ts>
	KS_FUNCTION Tuple<Ts...> zero(allocator * alloc, Tuple<Ts...> const& val)
	{
		return transform_Tuple(val, [alloc](auto const& elem) { return zero(alloc, elem); });
	}

	template <size_t Dim, class T>
	KS_FUNCTION tensor<Dim, T> zero(allocator * alloc, tensor<Dim, T> const& val)
	{
	...
	struct make_zero_t
	{
		static KS_FUNCTION T ofShape(allocator_base *, ks::Tuple<>) { return 0; }
	};
	...
	{
		template<size_t ...Indices>
		static KS_FUNCTION ks::Tuple<Ts...> ofShapeImpl(allocator_base * alloc, shape_t<ks::Tuple<Ts...>> const& shape, std::index_sequence<Indices...>) {
			return ks::make_Tuple(make_zero_t<Ts>::ofShape(alloc, ks::get<Indices>(shape))...);
		}

		static KS_FUNCTION ks::Tuple<Ts...> ofShape(allocator_base * alloc, shape_t<ks::Tuple<Ts...>> const& shape) {
	...
	{
		static KS_FUNCTION tensor<Dim, T> ofShape(allocator_base * alloc, shape_t<tensor<Dim, T>> const& shape) {
	...
	template<typename T>
	KS_FUNCTION T zeroOfShape(allocator_base * alloc, shape_t<T> const& shape)
	{
	...
	template <size_t Dim, class T>
	KS_FUNCTION tensor<Dim, T> ts_add(allocator * alloc, tensor<Dim, T> const& a, tensor<Dim, T> const& b)
	{
	...
	template <size_t Dim, class T>
	KS_FUNCTION tensor<Dim, T> ts_scale(allocator * alloc, Float val, tensor<Dim, T> const& t)
	{
		auto ret = tensor<Dim, T>::create(alloc, t.size());
		T* retdata = ret.data();
	...
	KS_INLINE_FUNCTION Float ts_neg(allocator *, Float d) { return -d; }

	template <class... Us>
	KS_INLINE_FUNCTION Tuple<Us...> ts_neg(allocator * alloc, Tuple<Us...> t) {
		return transform_Tuple(t, [alloc](auto const& elem) { return ts_neg(alloc, elem); });
	}

	template <size_t Dim, class T>
	KS_FUNCTION tensor<Dim, T> ts_neg(allocator * alloc, tensor<Dim, T> t) {
	...
	KS_FUNCTION vec<T> build(allocator * alloc, Integer size, F f)
	{
	...
	struct build_t
	{
		...
		static KS_FUNCTION void do_build(allocator * alloc, Size const& size, T** data, F f, HigherDimensionIndices ...higherDimensionIndices) {
	...
	KS_FUNCTION tensor<sizeof...(SizeTypes), T> build(allocator * alloc, ks::Tuple<SizeTypes...> size, F f)
	{
	...
	*/
	...
	struct sumbuild_t
	{
		...
		template<class T, class F, class Size, class ...HigherDimensionIndices>
		static KS_FUNCTION T do_sumbuild(allocator * alloc, Size const& size, F f, HigherDimensionIndices ...higherDimensionIndices) {
			int thisDimension = ks::get<sizeof...(HigherDimensionIndices)>(size);
		...
		template<class T, class F, class Size, class ...HigherDimensionIndices>
		static KS_FUNCTION void inplace_sumbuild(allocator * alloc, T* result, Size const& size, F f, HigherDimensionIndices ...higherDimensionIndices) {
			int thisDimension = ks::get<sizeof...(HigherDimensionIndices)>(size);
			for (int i = 0; i != thisDimension; ++i)
	...
	KS_FUNCTION T sumbuild(allocator * alloc, Size size, F f)
	{
	...
	KS_FUNCTION tensor<Dim, T> elementwise_map(allocator * alloc, tensor<Dim, T> const& t, F f)
	{
		auto ret = tensor<Dim, T>::create(alloc, t.size());
		T const* tdata = t.data();
		T* retdata = ret.data();
	...
	struct buildFromSparse_t
	{
		...
		static KS_FUNCTION void do_buildFromSparse(allocator * alloc, T* acc, LoopSize const& loopSize, F f, HigherDimensionIndices ...higherDimensionIndices) {
		...
		template<class T, class F, class LoopSize, class ...HigherDimensionIndices>
		static KS_FUNCTION void do_buildFromSparseTupled(allocator * alloc, T* acc, LoopSize const& loopSize, F f, HigherDimensionIndices ...higherDimensionIndices) {
			int thisDimension = ks::get<sizeof...(HigherDimensionIndices)>(loopSize);
			for (int i = 0; i != thisDimension; ++i)
	...
	struct buildFromSparse_t<1>
	{
		...
		static KS_FUNCTION void buildFromSparse_addOneIteration(tensor<Dim, T>* accElement, LambdaResultElementT const& lambdaResultElement) {
		...
		static KS_FUNCTION void buildFromSparseTupled_addOneIteration(T* acc, LambdaResultT const& lambdaResult, std::index_sequence<Indices...>) {
		...
		static KS_FUNCTION void do_buildFromSparse(allocator * alloc, tensor<Dim, T>* acc, LoopSize const& loopSize, F f, HigherDimensionIndices ...higherDimensionIndices) {
		...
		static KS_FUNCTION void do_buildFromSparseTupled(allocator * alloc, Tuple<Ts...>* acc, LoopSize const& loopSize, F f, HigherDimensionIndices ...higherDimensionIndices) {
			int thisDimension = get_dimension<sizeof...(HigherDimensionIndices)>(loopSize);
			KS_MARK(alloc, mark);
			for (int i = 0; i != thisDimension; ++i) {
	...
	KS_FUNCTION auto buildFromSparse(allocator * alloc, ResultShape const& resultShapes, LoopSize loopSize, F f)
	{
	...
	template<class ResultType, class ResultShape, class F, class LoopSize>
	KS_FUNCTION auto buildFromSparseTupled(allocator * alloc, ResultShape const& resultShapes, LoopSize loopSize, F f)
	{
		constexpr size_t Dim = dimension_of_tensor_index_type<LoopSize>::value;
		auto result = zeroOfShape<ResultType>(alloc, resultShapes);
	...
	KS_FUNCTION auto // Tuple<tensor<Dim, T>, tensor<Dim, B>>
	...
	KS_FUNCTION auto // tuple<tensor<Dim, dS>, dE>
	...
	KS_FUNCTION A fold(allocator * alloc, F f, A z, vec<T> v)
	{
		A acc = z;
	...
	KS_FUNCTION Tuple<dS, Tuple<dA, vec<dT>>> RFold(allocator * alloc, const dT &dummy, dS s_zero, F f, F_ f_, A acc, vec<T> v, dA dr) {
	...
	KS_FUNCTION dA FFold_recursive(allocator * alloc, Integer i, F f, A acc, vec<T> v, F_ f_, dA dacc, vec<dT> dv) {
		if (i == v.size()) {
			return dacc;
		} else {
	...
	template <class T, class F, class F_, class A, class dA, class dT>
	KS_FUNCTION dA FFold(allocator * alloc, F f, A acc, vec<T> v, F_ f_, dA dacc, vec<dT> dv) {
	...
	KS_FUNCTION vec<T> Vec_init(allocator * alloc, T arg0, ArgTypes... args)
	{
	...
	KS_FUNCTION T delta(allocator * alloc, IndexT i, IndexT j, T val)
	{
		return (i == j) ? val : zero(alloc, val);
	}

	template <class SizeType, class T>
	KS_FUNCTION auto constVec(allocator * alloc, SizeType size, T val)
	{
	...
	template <class SizeType, class T>
	KS_FUNCTION auto deltaVec(allocator * alloc, SizeType size, SizeType index, T val)
	{
		constexpr size_t Dim = dimension_of_tensor_index_type<SizeType>::value;
	...
	template <class F>
	KS_FUNCTION auto diag(allocator * alloc, int rows, int cols, F f)
	{
		KS_ASSERT(rows == cols);
	...
	// sum of elements
	template <size_t Dim, class T>
	KS_FUNCTION T sum(allocator * alloc, tensor<Dim, T> const& t)
	{
		int ne = t.num_elements();
		KS_ASSERT(ne > 0);
	...
	template <class T>
	KS_INLINE_FUNCTION Bool eq(T t1, T t2)
	{
		return t1 == t2;
	}

	template <class T>
	KS_INLINE_FUNCTION Bool ne(T t1, T t2)
	{
		return t1 != t2;
	}

	template<size_t I, size_t Dim, typename TupleType>
	KS_FUNCTION auto unzip_element(allocator * alloc, tensor<Dim, TupleType> const& t)
	{
	...
	KS_FUNCTION auto unzip_impl(allocator * alloc, tensor<Dim, TupleType> const& t, std::index_sequence<Indices...>)
	{
		return ks::make_Tuple(unzip_element<Indices>(alloc, t)...);
	}
	template <size_t Dim, class... Types>
	KS_FUNCTION auto unzip(allocator * alloc, tensor<Dim, Tuple<Types...>> const& t)
	{
...
#define $BENCH$al$d$d$bf$b(alloc, FUN) ks::benchmark(ks::repeat([&]() { \
	...
	// ===============================  Dot ===========================================
	KS_INLINE_FUNCTION Float ts_dot(Float t1, Float t2) { return t1 * t2; }

	template <class T>
	KS_INLINE_FUNCTION Float ts_dot(T t1, Tuple<> t2)
	{
		return 0.0;
	}

	template <class T>
	KS_INLINE_FUNCTION Float ts_dot(T t1, Tuple<T> t2)
	{
		return ts_dot(t1,ks::get<0>(t2));
	}

	template <class T0, class... Ts, class U0, class... Us>
	KS_INLINE_FUNCTION Float ts_dot(Tuple<T0, Ts...> t1, Tuple<U0, Us...> t2)
	{
		return ts_dot(head(t1), head(t2)) + ts_dot(tail(t1), tail(t2));
	}

	template <size_t Dim, class T1, class T2>
	KS_INLINE_FUNCTION Float ts_dot(tensor<Dim, T1> t1, tensor<Dim, T2> t2)
	{
		Float ret = 0;
  ...
  //  reverse mode generated code is correct.
	...
	KS_FUNCTION Float $check(allocator * alloc, Functor f, RevFunctor rev_f, X x, Dx dx, Df df)
	{
