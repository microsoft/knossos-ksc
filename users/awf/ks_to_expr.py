...
# Generate expressions for "Hashing modulo alpha equivalence", PLDI2021
...
def convert_expr(expr: ksc.Expr) -> Term:
    if isinstance(expr, ksc.Const):
        ...
        assert isinstance(expr.vars, ksc.Var)
        return App(
            Lam(var_name=convert_expr(expr.vars).name, body=convert_expr(expr.body)),
...
def main():
    ...
    exprs = parse_ks_file(in_path)

    exprs = map(detuple_lets, exprs)
