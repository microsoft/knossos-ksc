...
anf (L.Expr tye ex) = case ex of
  ...
  L.Tuple ts -> do
    anfArgs <- mapM letAnf ts
    return  $ 
      (foldr (.) id (map fst anfArgs)) $ L.Expr tye $ L.Tuple (map snd anfArgs)
  L.Lam x e -> do
    anfe <- anf e
...
letAnf e@(L.Expr tye ex) = do
  ve   <- freshVar
  anfe <- anf e
  return (\body@(L.Expr tybody bodyx) -> L.Expr tybody (L.Let (L.TVar tye ve) anfe body), L.Expr (L.typeOf anfe) (L.Var ve))

anfDefs :: [L.Def] -> [L.Def]
anfDefs defs = map anfDef defs

anfDef :: L.Def -> L.Def
anfDef (L.Def (L.TFun ty f) vars expr) = L.Def (L.TFun ty f) vars $ runM $ anf expr
...
cgenDefE (L.Def f@(L.TFun tyf _) vars expr) =
  ...
  let (cDecl, cVar) = runM $ cgenExpr expr in
    (  cgenType tyf `spc` cgenFun f
    ++ "("
    ++ intercalate
         ", "
...
cgenExprR (L.Expr ty ex) = case ex of
  ...
  L.Lam (L.TVar tv v) body -> do
    ...
    return (
        cgenType ty `spc` l ++ " = [&](" ++ cgenType tv `spc` cgenVar v ++ ") { " ++ cE ++ 
        "   return "++ vE ++"; };\n"
        , l
        )
  ...
  L.If c texpr fexpr -> do
    ...
    return (   declc -- emit condition generation
            ++ cgenType ty `spc` cret ++ ";\n" -- emit decl for "return" type
            ++ "if ("++vc++") {" 
            ++ "  " ++ declt ++ ";\n" -- compute true value
            ++ "  " ++ cret ++ "=" ++ vc ++ ";\n" -- assign to "return"
            ++ "} else {\n" -- else
