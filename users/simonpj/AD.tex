...
{\bf Semantics of linear maps}
  ...
  \begin{array}{rcl}
    ...
    \sigma(m) \lmapply x & = & \sum_{i=1}^{size(x)} (m_1 \lmapply x)[i] \\
    \lmzero \lmapply x  & = & 0 \\
    \lmone \lmapply x & = & x \\
...
%    \lmlamt{f} \lmapply g & = & \Sigma_i f(i) \lmapply g(i) \\
    ...
    \lmbuildt(n,f) \lmapply x & = & \sum_{i=1}^n (f\; i) \lmapply x[i]
   \end{array}
  $$
\\[3mm]
...
{\bf Derivatives of built-in functions} \\
      ...
      \begin{array}{rcl}
        ...
        \gradf{\indexfun{\cdot}{\cdot}}(v,i) & = & ??? \\[2mm]
        \gradf{\sumfun} & :: & Field \; t \darrow \vector{t} \to (\vector{t} \linto t) \\
        \gradf{\sumfun}(v) & = & \lmbuildt( size(v),\; \lambda i. \lmone ) \\[2mm]
...
\section{Old stuff about Build and lambda}
