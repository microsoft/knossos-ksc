...
whose elements have type $T$ (Figure~\ref{fig:syntax}).  \awf{Mention whether we can have matrix as vec of vec}
...
\item $\sumfun :: Field \; t \darrow \vector{t} \to t$ to add up the elements of a vector.
  ...
  \simon{I don't think so!  $\lmbuildt$ is a linear map, so you can't apply it to $v$.
    ...
    defined using $\sumfun$!}\awf{note mainly to self, better to have general fold here?}
...
\begin{itemize}
\item Do we need $\lmone$? After all $\lmscalar{1}$ does the same job.  But asking if $k=1$ is dodgy when $k$ is a float. \awf{No, perfectly fine to ask if a float is 1 --- the nearby floats are far away, and there's no other float $f$ such that $\lmscalar f = \lmone$.}
...
\item Construct the full Jacobian $\gradf{f}$, and transposed full Jacobian $\gradft{f}$,
  using the tranformations in Figure~\ref{fig:ad}. \awf{gradft cute, but $\nabla^\top\!\! f$ is more conventional}
