...
\newcommand{\gradft}[1]{\nabla^{\top}\!\mathit{#1}}  % Full Jacobian
...
%  \deltafun & :: & Field \; t \darrow (\nat,\; \nat) \to t & \mbox{Delta-function} \\
  ...
  \sumfunname & :: & Field \; t \darrow \vector{t} \to t & \mbox{Sum a vector} \\
...
{\bf Derivatives of built-in functions}
      ...
      \begin{array}{rcl}
        ...
        \gradf{\indexfun{\cdot}{\cdot}}(v,i) & = & \lmbuildt( \sizefun(v),
               \lambda j. \mbox{\lstinline|if|}\, i=j \,\mbox{\lstinline|then|}\, \lmone  \, \mbox{\lstinline|else|}\, \lmzero ) \lmjoin \lmzero \\[2mm]
...
%    \\
    \multicolumn{5}{l}{\mbox{\bf NB: We expect to have only $\lmlam{\cdot} / \lmlamt{\cdot}$ or $\lmbuild / \lmbuildt$, but not both}} \\
...
\item Theorem: $\forall (m :: S \linto T).\, m \lmapply 0 = 0$.  That is, all linear maps
pass through the origin.  Proof: property (LM2) with $k=0$.  Note that the function
...
\def\lmtomat{\mathcal M}% awf: define macros when first introduced in text, not at top of file
...
\awf{possibly good to get this right up front here:
The operations $\lmjoin$ and $\lmpair$ defined below then become horizontal and vertical matrix concatenation.
I.e., if $\lmtomat a$ is the matrix representation of linear map $a$, then  $\lmtomat (p \lmjoin q) = [\lmtomat p \mid \lmtomat q]$
AHA: just found figure 3 -- when doc settles down, might be good to get that figure earlier.
}\simon{I've added the previous para; we can sort out figure placement later.  OK?  If so, delete comments.}
...
$\gradf{\indexfun{\cdot}{\cdot}}$ are correct! 
